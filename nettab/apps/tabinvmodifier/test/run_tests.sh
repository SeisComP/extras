...
setup () {
	~/seiscomp/bin/seiscomp start spread scmaster
...
~/seiscomp/bin/seiscomp exec fdsnxml2inv --debug ge_some.xml | xmllint --format - > ge_inv.xml
...
~/seiscomp/bin/seiscomp exec tabinvmodifier -r ge_mod.rules --inventory-db ge_inv.xml -o ge_new.xml
...
Try --help for help
EOF
	~/seiscomp/bin/seiscomp exec tabinvmodifier >1 2>&1
	diff -q expected 1
	if [ $? -ne 0 ] ; then
		diff -s expected 1
		exit 1
...
Try --help for help
EOF
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r rules1 --inventory-db 4a_inv.xml >1 2>&1
	diff expected 1
	if [ $? -ne 0 ] ; then
		exit 1
	fi
	...
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r rules0 >1 2>&1
	if [ $? -ne 0 ] ; then
		cat 1
        	exit 1
	fi
...
EOF
	...
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r $f >1 2>&1
	diff expected 1
	if [ $? -ne 0 ] ; then
		exit 1
	fi
...
# Wrong case on attribute names. Based on Valentino L.'s example.
	...
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r rules1 >1 2>&1

# First version (Nov 2016) threw KeyError:
#
#   File "/home/pevans/seiscomp/bin/tabinvmodifier", line 263, in _modifyInventory
...
test_online () {
	...
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r rules2 --debug
}

test_offline () {
# Expect success - set netClass and archive on <network>, and archive on <stations>.
	echo "test_offline"
	~/seiscomp/bin/seiscomp exec tabinvmodifier -r rules2 --inventory-db 4a_inv.xml -o 4a_new.xml
